...
import static benchmarkold.Operation.GET;
import static benchmarkold.Operation.PUT;
import static benchmarkold.Operation.REMOVE;
...
public abstract class Client implements Callable<Stats>, Serializable {
    ...
    private transient KeyGenerator generator;
    ...
    protected void load(int size) {
        ...
        long keys = config.getRecordCount() / config.getClients();
        long start = min + id * keys;
        long end = start + keys;
        for (long i = start; i < end; i++) {
            Map.Entry entry = db.cast(i, data);
            db.put(entry.getKey(), entry.getValue());
            System.out.println("Loading key = " + i);
        }
    }
