...
PID=$(cat "${PID_FILE}");
if [ -z "${PID}" ]; then
    echo "Process id for worker instance is written to location: {$PID_FILE}"
    $RUN_JAVA $JAVA_OPTS benchmark.Worker &
    echo $! > ${PID_FILE}
else
    echo "Another worker instance is already started in this folder. To start a new instance, please unzip benchmark.zip/tar.gz in a new folder."
    exit 0
fi
